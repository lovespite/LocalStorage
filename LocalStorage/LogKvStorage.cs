...
public class LogKvStorage : KeyValueStorage, IDisposable
{
    ...
    public override string Name => Path.GetFileNameWithoutExtension(_filePath);
    // 内存索引
    ...
    private LogKvStorage(ConcurrentDictionary<string, string> cache, FileStream fileStream)
    {
        _fileStream = fileStream;
        _memoryIndex = new ConcurrentDictionary<string, string>(cache);
        _filePath = fileStream.Name;
    }

    public static LogKvStorage Open(string filePath)
    {
        ...
        var storage = new LogKvStorage(memoryIndex, fs);

        if (fs.Length > 0) Replay(fs, memoryIndex);
    ...
    public static Task<LogKvStorage> OpenAsync(string filePath) => Task.Run(() => Open(filePath));

    private static void Replay(Stream fileStream, ConcurrentDictionary<string, string> memoryIndex)
    {
        ...
        try
        {
            ...
            {
                // 记录当前记录开始的位置
                long currentRecordStart = fileStream.Position;
                ...
                {
                    memoryIndex.TryRemove(key, out _);
                }
                else
                {
                    // 遇到未知指令，认为是文件损坏，停止读取
                    ...
                    fileStream.Position = lastValidPosition;
                    fileStream.SetLength(lastValidPosition);
                    return;
                }

                // 记录成功解析完一条记录后的位置
                lastValidPosition = fileStream.Position;
            }
        }
        catch (EndOfStreamException)
        {
            // 文件截断（通常发生在只有一半写入时）
            // 回滚到最后一次成功的位置
            Debug.WriteLine($"[LogKvStorage] Unexpected EOF. Truncating from {fileStream.Length} to {lastValidPosition}.");
            fileStream.Position = lastValidPosition;
            fileStream.SetLength(lastValidPosition);
        }
        catch (Exception ex)
    ...
    {
        return Task.FromResult(_memoryIndex.ContainsKey(key));
    }
